-- 1) Aceitar novo status 'aguardando' em monitoramento_rotas
...
AS $$
BEGIN
  IF NEW.status NOT IN ('aguardando','ativa','finalizada','pausada') THEN
...
-- 2) RPC de provisionamento automático da Torre para uma data
CREATE OR REPLACE FUNCTION public.provisionar_torre_dia(p_data date DEFAULT CURRENT_DATE)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  LOOP
    -- Agrega NFs do veículo por CNPJ, unindo com melhor roteirização (por cobertura),
    -- endereços cadastrais e raio customizado do destinatário.
    WITH nfs AS (
      SELECT nf.cnpj_destinatario,
             nf.dest_razao_social,
             nf.dest_logradouro, nf.dest_numero, nf.dest_bairro,
             nf.dest_cidade, nf.dest_uf,
             nf.peso_bruto, nf.volume_m3,
             vn.carga_origem_id
    ...
    cargas AS (
      SELECT DISTINCT carga_origem_id AS carga_id FROM nfs WHERE carga_origem_id IS NOT NULL
    ),
    rots AS (
      SELECT r.id, r.created_at
        FROM public.roteirizacoes r
       WHERE r.carga_id IN (SELECT carga_id FROM cargas)
    ),
    rot_paradas AS (
      SELECT rp.*, regexp_replace(coalesce(rp.cnpj_destinatario,''),'\D','','g') AS cnpj_n
        FROM public.roteirizacao_paradas rp
       WHERE rp.roteirizacao_id IN (SELECT id FROM rots)
    ),
    rot_rank AS (
      SELECT r.id,
             count(*) FILTER (WHERE rp.cnpj_n IN (SELECT cnpj_n FROM grouped WHERE cnpj_n <> '')) AS cobertura,
             r.created_at
        FROM rots r
        LEFT JOIN rot_paradas rp ON rp.roteirizacao_id = r.id
       GROUP BY r.id, r.created_at
       ORDER BY cobertura DESC, r.created_at DESC
       LIMIT 1
    ),
    rot_best AS (
      SELECT rp.*
        FROM rot_paradas rp
       WHERE rp.roteirizacao_id = (SELECT id FROM rot_rank)
    ),
    -- Endereço/coord do cadastro mestre (destinatario_enderecos.principal)
    end_mestre AS (
      SELECT DISTINCT ON (regexp_replace(d.cnpj_cpf,'\D','','g'))
             regexp_replace(d.cnpj_cpf,'\D','','g') AS cnpj_n,
             de.latitude AS lat, de.longitude AS lng,
             d.raio_geofence_metros
       ...
       ORDER BY regexp_replace(d.cnpj_cpf,'\D','','g'),
                de.principal DESC NULLS LAST, de.updated_at DESC NULLS LAST
    ),
    merged AS (
      SELECT g.cnpj, g.cnpj_n, g.razao, g.log, g.num, g.bai, g.cid, g.uf,
             g.qtd_nfs, g.peso, g.vol,
             rb.ordem AS rot_ordem,
             rb.razao_social AS rot_razao,
             rb.endereco_completo AS rot_end,
             coalesce(em.lat, rb.latitude) AS lat,
             coalesce(em.lng, rb.longitude) AS lng,
             rb.total_nfs AS rot_total_nfs,
             rb.total_caixas AS rot_total_caixas,
             rb.peso_total_kg AS rot_peso, rb.volume_total_m3 AS rot_vol,
             em.raio_geofence_metros
        FROM grouped g
    ...
    SELECT count(*)::int INTO v_total_paradas FROM ordered;
    ...
    INSERT INTO public.monitoramento_rotas (
      veiculo_id, motorista, placa, data, status, total_paradas
    ) VALUES (
      v_veic.id, v_veic.motorista, v_veic.placa, v_veic.data, 'aguardando', v_total_paradas
    )
    RETURNING id INTO v_rota_id;

    -- Re-executa CTE para inserir paradas (não pode reaproveitar CTE após SELECT)
    ...
    rot_rank AS (
      SELECT r.id
        FROM rots r LEFT JOIN rot_paradas rp ON rp.roteirizacao_id = r.id
       GROUP BY r.id, r.created_at
       ORDER BY count(*) FILTER (WHERE rp.cnpj_n IN (SELECT cnpj_n FROM grouped WHERE cnpj_n <> '')) DESC,
                r.created_at DESC
       LIMIT 1
    ),
    ...
    merged AS (
      SELECT g.cnpj, g.cnpj_n, g.razao, g.log, g.num, g.bai, g.cid, g.uf,
             g.qtd_nfs, g.peso, g.vol,
             rb.ordem AS rot_ordem, rb.razao_social AS rot_razao,
             rb.endereco_completo AS rot_end,
             coalesce(em.lat, rb.latitude) AS lat,
             coalesce(em.lng, rb.longitude) AS lng,
  ...
  RETURN jsonb_build_object('status','ok','criadas', v_created, 'ignoradas', v_skipped, 'data', p_data);
END;
$$;

GRANT EXECUTE ON FUNCTION public.provisionar_torre_dia(date) TO authenticated;

-- 3) Trigger: primeiro ping GPS promove rota 'aguardando' -> 'ativa'
CREATE OR REPLACE FUNCTION public.fn_promover_rota_por_gps()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  UPDATE public.monitoramento_rotas
     SET status = 'ativa',
         ultima_lat = NEW.latitude,
         ultima_lng = NEW.longitude,
         ultima_atualizacao = NEW.registrado_em
   WHERE id = NEW.monitoramento_rota_id
     AND status = 'aguardando';
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_promover_rota_por_gps ON public.posicoes_gps;
CREATE TRIGGER trg_promover_rota_por_gps
AFTER INSERT ON public.posicoes_gps
FOR EACH ROW
WHEN (NEW.heartbeat = false)
EXECUTE FUNCTION public.fn_promover_rota_por_gps();