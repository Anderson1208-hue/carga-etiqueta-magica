...
ALTER TABLE public.etiquetas 
  ...
  ADD COLUMN IF NOT EXISTS divergencia_por uuid,
  ADD COLUMN IF NOT EXISTS divergencia_em timestamptz;

-- Update progress function to exclude divergencia from totals
...
AS $$
  SELECT jsonb_build_object(
    ...
    'divergencias', COALESCE(SUM(counts.divergencias), 0),
    'nfs', COALESCE(jsonb_agg(
      jsonb_build_object(
        ...
        'divergencias', counts.divergencias
  ...
  FROM (
    SELECT 
      e.numero_nf,
      COUNT(*) FILTER (WHERE e.status != 'divergencia')::int AS total,
      ...
      COUNT(*) FILTER (WHERE e.status = 'divergencia')::int AS divergencias
    FROM etiquetas e
