  ...
  WHERE coalesce(nf.data_emissao, nf.created_at::date) >= (current_date - p_dias)
    ...
    AND NOT EXISTS (
      ...
      WHERE p.cnpj_embarcador = regexp_replace(nf.cnpj_emitente, '\D', '', 'g')
        AND btrim(p.codigo) = btrim(i.c_prod)
    )
  GROUP BY 1, i.c_prod
  ORDER BY count(*) DESC, i.c_prod;
$$;

REVOKE ALL ON FUNCTION public.produtos_pendentes_cadastro(integer, text) FROM public;
GRANT EXECUTE ON FUNCTION public.produtos_pendentes_cadastro(integer, text) TO authenticated;

CREATE OR REPLACE FUNCTION public.aplicar_cubagem_produtos_nf(p_dias integer DEFAULT 7, p_simular boolean DEFAULT true)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
REVOKE ALL ON FUNCTION public.aplicar_cubagem_produtos_nf(integer, boolean) FROM public;
GRANT EXECUTE ON FUNCTION public.aplicar_cubagem_produtos_nf(integer, boolean) TO authenticated;