...
BEGIN
  ...
  v_tipo_carga := COALESCE(NULLIF(payload #>> '{carga,tipo_carga}', ''), 'SECA');
  ...
  IF v_batch_id IS NOT NULL THEN
    ...
    IF v_existing_carga_id IS NOT NULL THEN
      RETURN jsonb_build_object('status','already_processed','carga_id',v_existing_carga_id);
    END IF;
  END IF;

  INSERT INTO public.cargas (motorista, placa, observacao, data, created_by, import_batch_id, tipo_carga)
  VALUES (
  ...
  FOR v_nf IN
    ...
    FROM jsonb_to_recordset(COALESCE(payload->'nfs','[]'::jsonb)) AS x(
      chave_acesso text, numero_nf text, serie text, cnpj_emitente text, razao_social_emitente text,
      ...
      volume_m3 numeric, valor_nf numeric, itens jsonb, etiquetas jsonb
    )
  LOOP
    ...
    INSERT INTO public.notas_fiscais (
      carga_id, chave_acesso, numero_nf, serie, cnpj_emitente, razao_social_emitente,
      data_emissao, cnpj_destinatario, dest_razao_social, dest_logradouro,
      dest_numero, dest_bairro, dest_cidade, dest_uf, dest_cep,
      peso_bruto, peso_liquido, volume_m3, valor_nf
    )
    VALUES (
      v_carga_id, v_nf.chave_acesso, v_nf.numero_nf, v_nf.serie, v_nf.cnpj_emitente,
      ...
      COALESCE(v_nf.volume_m3, 0), v_nf.valor_nf
    )
  ...
  IF v_importados = 0 THEN
    ...
    RETURN jsonb_build_object(
      'status','no_valid_nfs','total_enviados',v_total_xml,
      'importados',v_importados,'ignorados_duplicidade',v_duplicados,
      'duplicados',v_resumo_duplicados
    );
  END IF;

  RETURN jsonb_build_object(
    'status','ok','carga_id',v_carga_id,'total_enviados',v_total_xml,
    'importados',v_importados,'ignorados_duplicidade',v_duplicados,
    'duplicados',v_resumo_duplicados
  );
END;
...
BEGIN
  ...
  LOOP
    ...
    IF v_nf_id IS NULL THEN
      ...
      END IF;
      -- Backfill serie/valor_nf quando NF já existia mas estava sem
      UPDATE public.notas_fiscais
         SET serie = COALESCE(serie, v_nf.serie),
             valor_nf = COALESCE(valor_nf, v_nf.valor_nf)
       WHERE chave_acesso = v_nf.chave_acesso;
      CONTINUE;
    END IF;
