...
-- Novas colunas de ambiguidade
ALTER TABLE public.sugestoes_coordenada
  ADD COLUMN IF NOT EXISTS ambigua boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS motivo_ambiguidade text;

-- Recriar RPC com nova assinatura (contadores)
DROP FUNCTION IF EXISTS public.gerar_sugestoes_dwell_factual();
...
BEGIN
  ...
  LOOP
    -- Cooldown: rejeitada nos últimos 60 dias
    IF EXISTS (
    ...
    -- Pré-selecionar paradas que atendem evidência de visita (dwell 5-120min)
    -- Calcular centroide POR ROTA com accuracy <=30m; fallback 50m se rota não tiver amostra
    v_acc_limit := 30;
    ...
    -- Considera apenas paradas com dwell entre 5 e 120 min
    FOR rota IN
    ...
    LOOP
      -- Centroide da parada com accuracy 30m; velocidade média
      INSERT INTO _tmp_route_centroids
      SELECT rota.monitoramento_rota_id, rota.veiculo_id,
             avg(p.latitude), avg(p.longitude),
             min(p.registrado_em), max(p.registrado_em),
             count(*)::int,
             COALESCE(avg(p.velocidade), 0)
      ...
      HAVING count(*) >= 5 AND COALESCE(avg(p.velocidade), 0) <= 3;

      -- Fallback 50m se nada entrou para esta parada
      IF NOT FOUND THEN
        INSERT INTO _tmp_route_centroids
        SELECT rota.monitoramento_rota_id, rota.veiculo_id,
               avg(p.latitude), avg(p.longitude),
    ...
    -- Recorrência: precisamos de >=2 rotas com centroide
    SELECT count(*) INTO v_rotas FROM _tmp_route_centroids;
    IF v_rotas < 2 THEN
      -- Distinguir motivo: se havia paradas mas nenhuma com accuracy/dwell → accuracy/dwell
      IF EXISTS (
    ...
    -- Consistência: distância máxima par-a-par entre centroides <= 100m
    ...
    -- Sugestão final = mediana das rotas
    SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY lat),
           percentile_cont(0.5) WITHIN GROUP (ORDER BY lng),
           sum(n_pings), count(DISTINCT veiculo_id), min(first_ts), max(last_ts)
      INTO v_lat, v_lng, v_pings, v_placas, v_first, v_last
      FROM _tmp_route_centroids;

    -- Raio p90 usando pings agregados válidos (usa o mesmo accuracy limit efetivo por rota)
    SELECT percentile_cont(0.9) WITHIN GROUP (ORDER BY
     ...
     WHERE mp.destinatario_id = r.dest_id
       AND mp.status IN ('finalizada','em_atendimento')
       AND mp.horario_chegada >= now() - interval '30 days'
       AND EXTRACT(EPOCH FROM (COALESCE(mp.horario_saida, mp.horario_chegada) - mp.horario_chegada))/60 BETWEEN 5 AND 120;

    -- Endereço principal atual
    ...
    -- Ambiguidade: outra parada DA MESMA ROTA a <200m do cluster proposto
    v_ambigua := false;
    v_motivo_amb := NULL;
    SELECT true, format('%s parada(s) da mesma rota a <200m do cluster', count(*))
     ...
     WHERE mp2.monitoramento_rota_id IN (SELECT rota_id FROM _tmp_route_centroids)
       AND mp2.destinatario_id IS NOT NULL
       AND mp2.destinatario_id <> r.dest_id
    ...
    -- P4: outros clientes cadastrados <200m
