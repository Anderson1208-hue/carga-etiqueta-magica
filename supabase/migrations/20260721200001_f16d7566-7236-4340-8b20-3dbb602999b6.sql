...
BEGIN
  ...
  SELECT COALESCE(raio_padrao_metros, 200) INTO v_raio_padrao
  FROM public.monitoramento_config
  LIMIT 1;
  v_raio_padrao := COALESCE(v_raio_padrao, 200);
  ...
  SELECT v.id, v.placa, v.motorista, v.data
   ...
   WHERE v.id = p_veiculo_id
     ...
     AND NOT EXISTS (
       SELECT 1
         FROM public.monitoramento_rotas r
         LEFT JOIN public.veiculos rv ON rv.id = r.veiculo_id
        WHERE (r.veiculo_id = v.id OR upper(regexp_replace(coalesce(r.placa,''),'[^A-Za-z0-9]','','g'))
          ...
          AND (rv.id IS NULL OR rv.prestacao_contas_em IS NULL)
          AND (
            r.veiculo_id = v.id
            OR r.data = v.data
            OR r.status IN ('ativa','pausada')
          )
     );
  ...
  SELECT v_rota_id, ordem_final, cnpj,
         ...
         lat, lng,
         coalesce(raio_cli, v_raio_padrao),
...
WITH cfg AS (
  SELECT COALESCE(raio_padrao_metros, 200) AS raio_padrao_metros
  FROM public.monitoramento_config
  LIMIT 1
)
UPDATE public.monitoramento_paradas mp
SET raio_geofence_metros = cfg.raio_padrao_metros
FROM public.monitoramento_rotas mr, cfg
WHERE mr.id = mp.monitoramento_rota_id
  AND mr.data >= CURRENT_DATE - INTERVAL '2 days'
  AND mr.status IN ('aguardando','ativa','pausada')
  AND mp.raio_geofence_metros = 100
  AND cfg.raio_padrao_metros > 100;