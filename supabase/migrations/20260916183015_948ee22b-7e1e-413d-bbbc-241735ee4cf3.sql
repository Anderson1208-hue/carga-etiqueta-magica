ALTER TABLE public.veiculos
  ADD COLUMN IF NOT EXISTS conferencia_interna_fechada_em timestamptz,
  ADD COLUMN IF NOT EXISTS conferencia_interna_fechada_por uuid,
  ADD COLUMN IF NOT EXISTS conferencia_interna_com_pendencia boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS conferencia_interna_pendencia_motivo text;

CREATE OR REPLACE FUNCTION public.conferencia_interna_status_veiculo(p_veiculo_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  SELECT id, placa, conferencia_interna_fechada_em, conferencia_interna_fechada_por,
         conferencia_interna_com_pendencia, conferencia_interna_pendencia_motivo
    INTO v_veiculo
  ...
  SELECT jsonb_build_object(
    ...
    'fechada_em', v_veiculo.conferencia_interna_fechada_em,
    'com_pendencia', v_veiculo.conferencia_interna_com_pendencia,
    'pendencia_motivo', v_veiculo.conferencia_interna_pendencia_motivo,
...
CREATE OR REPLACE FUNCTION public.fechar_conferencia_interna_veiculo(
  p_veiculo_id uuid,
...
BEGIN
  ...
  v_status := public.conferencia_interna_status_veiculo(p_veiculo_id);
  v_faltando := (v_status->>'faltando_escopo')::int;

  IF v_faltando > 0 THEN
    IF NOT p_forcar THEN
      RAISE EXCEPTION 'Faltam % nota(s) do escopo IBAC para fechar a expedição', v_faltando;
    END IF;
  ...
  SET conferencia_interna_fechada_em = now(),
      conferencia_interna_fechada_por = auth.uid(),
      conferencia_interna_com_pendencia = (v_faltando > 0),
      conferencia_interna_pendencia_motivo = CASE WHEN v_faltando > 0 THEN btrim(p_motivo) ELSE NULL END,
      updated_at = now()
  WHERE id = p_veiculo_id;

  RETURN public.conferencia_interna_status_veiculo(p_veiculo_id);
END;
$$;

GRANT EXECUTE ON FUNCTION public.conferencia_interna_status_veiculo(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fechar_conferencia_interna_veiculo(uuid, boolean, text) TO authenticated;