     ...
     AND NOT EXISTS (
       SELECT 1
        ...
        WHERE (r.veiculo_id = v.id OR upper(regexp_replace(coalesce(r.placa,''),'[^A-Za-z0-9]','','g'))
          ...
          -- Não deixa uma rota antiga já baixada bloquear a placa reaproveitada hoje.
          AND (rv.id IS NULL OR rv.prestacao_contas_em IS NULL)
          -- Bloqueia somente duplicidade real: mesmo veículo, mesma data ou rota ainda em andamento.
          AND (
            r.veiculo_id = v.id
            OR r.data = v.data
