...
export default function Roteirizacao() {
  ...
  async function handleEmplacar() {
    ...
    try {
      ...
      // OBS: o status da Carga (Aberta/Fechada) é controlado manualmente
      // no módulo Cargas. Nenhum outro módulo deve alterá-lo automaticamente.
      ...
      for (const veiculo of veics) {
        const { data } = await supabase
          ...
          .select(`
            ...
            notas_fiscais!inner(numero_nf, dest_razao_social, dest_bairro, dest_cep, dest_logradouro, dest_numero, dest_cidade, dest_uf, peso_bruto, volume_m3, cnpj_destinatario, razao_social_emitente, data_emissao, itens_nf(q_com))
          `)
                        ...
                        >
                          ...
                          >
                            <div className="flex items-center gap-4">
                              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${v.pernoite ? "bg-blue-500/20" : "bg-primary/10"}`}>
                                <Truck className={`w-5 h-5 ${v.pernoite ? "text-blue-600 dark:text-blue-400" : "text-primary"}`} />
                              </div>
                              <div>
                                <p className="font-semibold flex items-center gap-2">
                                  {v.placa}
                                  {v.pernoite && (
                                    <Badge className="text-[10px] h-5 px-1.5 bg-blue-600 hover:bg-blue-700 text-white border-transparent">
                                      PERNOITE
                                    </Badge>
                                  )}
                                </p>
