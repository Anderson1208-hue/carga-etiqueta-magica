...
export default function Cargas() {
  ...
  async function loadCargas() {
    setLoading(true);
    try {
      // Usa RPC que agrega contagens no servidor (muito mais rápido e estável)
      const { data, error } = await supabase.rpc("get_cargas_com_contagens");
      if (error) throw error;

      const cargasWithCounts = (data || []).map((c: any) => ({
        id: c.id,
        data: c.data,
        placa: c.placa,
        ...
        operador_responsavel: c.operador_responsavel,
        tipo_carga: c.tipo_carga,
        _count: {
          nfs: Number(c.nfs_count) || 0,
          itens: Number(c.itens_count) || 0,
        },
      }));
