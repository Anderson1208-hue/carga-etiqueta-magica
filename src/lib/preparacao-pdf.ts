...
export function gerarPreparacaoPdf(nfs: NfPrepPdf[], mrLabel: string) {
  ...
  doc.setFontSize(10);
  doc.setFont("helvetica", "bold");
  doc.text(`TOTAL: ${totalEntregas} entregas  •  ${totalNfs} NFs  •  ${totalCaixas} caixas  •  ${totalPeso.toFixed(1)} kg  •  ${totalVolume.toFixed(2)} m³  •  ${fmtBRL(totalValor)}`, MARGIN, y + 3);  •  ${totalVolume.toFixed(2)} m³`, MARGIN, y + 3);
