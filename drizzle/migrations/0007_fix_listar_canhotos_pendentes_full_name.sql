...
RETURNS TABLE(baixa_id uuid, nf_id uuid, numero_nf text, dest_razao_social text, dest_cidade text, embarcador text, cnpj_emitente text, veiculo_id uuid, placa text, motorista text, data_rota date, registrado_em timestamp with time zone, ocorrencia text, origem text, motivo text, observacao text, marcado_em timestamp with time zone, marcado_por_nome text, dias_corridos integer, prazo_vencido boolean)
LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT b.id, b.nf_id, n.numero_nf, n.dest_razao_social, n.dest_cidade,
    ...
    coalesce(p.full_name, pr.full_name),
  ...
  WHERE b.canhoto_recuperado_em IS NULL
    AND coalesce(b.observacao, '') NOT ILIKE '%praxio%'
    AND (b.conferencia_status = 'canhoto_pendente'
         OR (b.ocorrencia = 'sem_canhoto' AND b.registrado_por IS NOT NULL
             AND b.foto_path IS NULL AND b.foto_recibo_path IS NULL))
