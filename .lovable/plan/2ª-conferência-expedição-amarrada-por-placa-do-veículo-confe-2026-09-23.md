...
1. **Etapa 1 — Separação:** bipagem dupla (código do cliente + QR da etiqueta), obrigatória para IBAC/IBAE (Cacau Show).
...
3. **O escopo obrigatório é IBAC/IBAE:** notas cujo emitente (CNPJ raiz) esteja ativo em `cnpj_envio_canhoto_auto`. Notas de outros embarcadores aparecem para conferência, mas não contam no fechamento e não travam o veículo.
...
## Situação atual
- As estruturas no banco (colunas em `veiculos` e as duas RPCs) já existem desde 16/09, sem uso operacional.
- A interface foi revertida a seu pedido; esta implantação reativa a tela da Etapa 2 por placa e os selos.
- Etapa 1 (bipagem dupla cliente → QR nossa, com o bloqueio de 1º bipe na nossa etiqueta) permanece exatamente como roda hoje.
